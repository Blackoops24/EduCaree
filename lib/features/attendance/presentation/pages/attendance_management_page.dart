...
class _AttendanceManagementPageState
    extends PersistentModuleState<AttendanceManagementPage> {
  ...
  void importState(Map<String, dynamic> data) {
    ...
    _staffAttendance
      ...
      ..addAll(
        (data['staff'] as List? ?? []).map(
          (e) => StaffAttendanceRecord.fromJson(
            Map<String, dynamic>.from(e as Map),
          ),
        ),
      );
    _biometricDevices
      ..clear()
      ..addAll(
        (data['devices'] as List? ?? []).map(
          (e) => BiometricDevice.fromJson(Map<String, dynamic>.from(e as Map)),
        ),
      );
    _faceRecords
      ..clear()
      ..addAll(
        (data['faces'] as List? ?? []).map(
          (e) => FaceRecognitionRecord.fromJson(
            Map<String, dynamic>.from(e as Map),
          ),
        ),
      );
  ...
  Widget _buildStudentAttendanceTab(BuildContext context) {
    return Column(
      ...
      children: [
        _buildSectionHeader(
          'Student Attendance',
          'Mark daily and monthly attendance.',
          action: () => _showStudentAttendanceDialog(context),
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return Card(
                      ...
                      child: ListTile(
                        ...
                        subtitle: Text(
                          '${record.date} • ${record.status} • ${record.className}',
                        ),
                        trailing: Row(
                          ...
                          children: [
                            ...
                            IconButton(
                              ...
                              onPressed: () => setState(
                                () => _studentAttendance.removeAt(index),
                              ),
                            ),
                          ],
                        ),
    ...
    return Column(
      ...
      children: [
        _buildSectionHeader(
          ...
          action: () => _showStaffAttendanceDialog(context),
          actionLabel: 'Mark Attendance',
        ),
        Expanded(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return Card(
                      ...
                      child: ListTile(
                        ...
                        subtitle: Text(
                          '${record.date} • ${record.status} • Check-in: ${record.checkIn}',
                        ),
                        trailing: Row(
                          mainAxisSize: MainAxisSize.min,
                          children: [
  ...
  Widget _buildBiometricTab(BuildContext context) {
    return Column(
      ...
      children: [
        _buildSectionHeader(
          'Biometric Devices',
          'Manage biometric attendance integration.',
          action: () => _showBiometricDialog(context),
                        ...
                        trailing: Row(
                          ...
                          children: [
                            ...
                            IconButton(
                              icon: const Icon(Icons.edit),
                              onPressed: () =>
                                  _showBiometricDialog(context, device: device),
                            ),
                            IconButton(
                              icon: const Icon(Icons.delete),
                              onPressed: () => setState(
                                () => _biometricDevices.removeAt(index),
                              ),
                            ),
                          ],
                        ),
    ...
    return Column(
      ...
      children: [
        _buildSectionHeader(
          ...
          action: () => _showFaceRecognitionDialog(context),
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return Card(
                      ...
                      child: ListTile(
                        ...
                        subtitle: Text(
                          '${record.timestamp} • Confidence: ${record.confidence}%',
                        ),
                        trailing: Row(
                          mainAxisSize: MainAxisSize.min,
                          children: [
                            ...
                            IconButton(
                              icon: const Icon(Icons.edit),
                              onPressed: () => _showFaceRecognitionDialog(
                                context,
                                record: record,
                              ),
                            ),
                            IconButton(
                              icon: const Icon(Icons.delete),
                              onPressed: () =>
                                  setState(() => _faceRecords.removeAt(index)),
                            ),
                          ],
                        ),
                      ),
          ...
          child: Column(
            ...
            children: [
              ...
              DropdownButtonFormField<String>(
                initialValue: status,
                items: ['Present', 'Absent', 'Leave', 'Late']
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () {
              ...
              if (name.isEmpty) {
                focusAndRevealController(context, studentNameController);
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(content: Text('Enter student name.')),
                );
                return;
              }
              setState(() {
                if (record == null) {
                  _studentAttendance.add(
                    StudentAttendanceRecord(
                      id: _studentAttendance.isEmpty
                          ? 1
                          : _studentAttendance.last.id + 1,
                      studentName: name,
  ...
  void _showStaffAttendanceDialog(
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        title: Text(
          record == null ? 'Mark Staff Attendance' : 'Edit Staff Attendance',
        ),
        content: SingleChildScrollView(
          child: Column(
            mainAxisSize: MainAxisSize.min,
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () {
              ...
              if (name.isEmpty) {
                focusAndRevealController(context, employeeNameController);
                ScaffoldMessenger.of(context).showSnackBar(
  ...
  void _showBiometricDialog(BuildContext context, {BiometricDevice? device}) {
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        title: Text(
          device == null ? 'Add Biometric Device' : 'Edit Biometric Device',
        ),
        content: SingleChildScrollView(
          child: Column(
            mainAxisSize: MainAxisSize.min,
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () {
              ...
              if (name.isEmpty) {
                focusAndRevealController(context, deviceNameController);
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(content: Text('Enter device name.')),
                );
                return;
              }
              setState(() {
                if (device == null) {
                  _biometricDevices.add(
                    BiometricDevice(
                      id: _biometricDevices.isEmpty
                          ? 1
                          : _biometricDevices.last.id + 1,
                      deviceName: name,
                      type: deviceType,
                      status: status,
  ...
  void _showFaceRecognitionDialog(
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        title: Text(
          record == null ? 'Face Recognition Record' : 'Edit Face Record',
        ),
        content: SingleChildScrollView(
          child: Column(
            mainAxisSize: MainAxisSize.min,
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () {
              ...
              if (name.isEmpty) {
                focusAndRevealController(context, personNameController);
                ScaffoldMessenger.of(context).showSnackBar(
              ...
              if (confidence == null || confidence < 0 || confidence > 100) {
                focusAndRevealController(context, confidenceController);
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(
                    content: Text('Confidence must be between 0 and 100.'),
                  ),
                );
                return;
              }
              ...
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(content: Text('Face record for $name added.')),
              );
              Navigator.pop(context);
            },
...
class StaffAttendanceRecord {
  ...
  factory StaffAttendanceRecord.fromJson(Map<String, dynamic> j) =>
      StaffAttendanceRecord(
        id: j['id'] as int,
...
class FaceRecognitionRecord {
  ...
  factory FaceRecognitionRecord.fromJson(Map<String, dynamic> j) =>
      FaceRecognitionRecord(
        id: j['id'] as int,
