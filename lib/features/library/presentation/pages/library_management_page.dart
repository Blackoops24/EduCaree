...
class _LibraryManagementPageState
    extends PersistentModuleState<LibraryManagementPage> {
  final List<BookCategory> _categories = [
    ...
    BookCategory(
      id: 2,
      name: 'Science',
      description: 'Science, technology, and reference books',
    ),
    BookCategory(
      id: 3,
      name: 'History',
    ...
    _categories
      ...
      ..addAll(
        (data['categories'] as List? ?? []).map(
          (e) => BookCategory.fromJson(Map<String, dynamic>.from(e as Map)),
        ),
      );
    _books
      ..clear()
      ..addAll(
        (data['books'] as List? ?? []).map(
          (e) => LibraryBook.fromJson(Map<String, dynamic>.from(e as Map)),
        ),
      );
    _issues
      ..clear()
      ..addAll(
        (data['issues'] as List? ?? []).map(
          (e) => BookIssue.fromJson(Map<String, dynamic>.from(e as Map)),
        ),
      );
    _fines
      ..clear()
      ..addAll(
        (data['fines'] as List? ?? []).map((e) {
  ...
  Widget _buildCategoryTab(BuildContext context) {
    return Column(
      ...
      children: [
        _buildSectionHeader(
          'Book Categories',
          'Manage categories used by the library.',
        ...
        Expanded(
          child: _categories.isEmpty
              ? const Center(
                  child: Text('No categories have been created yet.'),
                )
              : ListView.separated(
                              ...
                              onPressed: () async {
                                final confirmed =
                                    await showDeleteConfirmationDialog(
                                      ...
                                      message:
                                          'This will remove ${category.name} from book categories.',
                                    );
                                if (!confirmed) return;
    ...
    return Column(
      ...
      children: [
        _buildSectionHeader(
          ...
          action: () => _showBookDialog(context),
                                  ...
                                  onPressed: () async {
                                    final confirmed =
                                        await showDeleteConfirmationDialog(
                                          ...
                                          title: 'Delete book?',
                                          message:
                                              'This will remove ${book.title} from library inventory.',
                                        );
                                    if (!confirmed) return;
  ...
  Widget _buildIssueTab(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        _buildSectionHeader(
          'Issue Books',
          'Assign books to students and track due dates.',
          action: () => _showIssueDialog(context),
          actionLabel: 'Issue Book',
        ),
        Expanded(
          child: availableBooks.isEmpty
              ? const Center(
                  child: Text('No books are currently available for issue.'),
                )
              : ListView.separated(
                  padding: const EdgeInsets.symmetric(horizontal: 16),
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return Card(
                      ...
                      child: ListTile(
                        title: Text(book.title),
                        subtitle: Text(
                          '${book.author} • ${book.category} • Available: ${book.availableCopies}',
                        ),
                        trailing: ElevatedButton(
                          onPressed: () =>
                              _showIssueDialog(context, book: book),
  ...
  Widget _buildReturnTab(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        _buildSectionHeader(
          'Return Books',
          'Return issued books and calculate late fees.',
          action: () {},
          actionLabel: null,
        ),
        Expanded(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return Card(
                      ...
                      child: ListTile(
                        ...
                        subtitle: Text(
                          '${issue.studentName} • Due ${issue.dueDate} • ${overdueDays > 0 ? '$overdueDays days overdue' : 'On schedule'}',
                        ),
                        trailing: Row(
                          mainAxisSize: MainAxisSize.min,
                          children: [
                            IconButton(
                              icon: const Icon(Icons.edit),
                              onPressed: () =>
                                  _showIssueDialog(context, issue: issue),
                            ),
                            ElevatedButton(
  ...
  Widget _buildFinesTab(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        _buildSectionHeader(
          'Fine Calculation',
          'Review overdue fines and settle dues.',
        ),
        Expanded(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return Card(
                      ...
                      child: ListTile(
                        ...
                        subtitle: Text(
                          '${fine.issue.studentName} • ₹${fine.amount.toStringAsFixed(0)}',
                        ),
                        trailing: Row(
                          mainAxisSize: MainAxisSize.min,
                          children: [
                            IconButton(
                              icon: const Icon(Icons.edit),
                              onPressed: () => _showFineDialog(context, fine),
                            ),
                            ElevatedButton(
  ...
  Widget _buildReportsTab(BuildContext context) {
    ...
    final overdueIssues = _issues
        .where(
          (issue) =>
              !_isReturned(issue) && _calculateOverdueDays(issue.dueDate) > 0,
        )
        .toList();
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          _buildSectionHeader(
            'Library Reports',
            'Issued books and overdue status at a glance.',
          ),
          ...
          ..._issues.map(
            (issue) => Card(
              ...
              child: ListTile(
                title: Text(issue.bookTitle),
                subtitle: Text(
                  '${issue.studentName} • Issued ${issue.issueDate} • Due ${issue.dueDate}',
                ),
                      ...
                      .map(
                        (issue) => Card(
                          ...
                          child: ListTile(
                            title: Text(issue.bookTitle),
                            subtitle: Text(
                              '${issue.studentName} • Due ${issue.dueDate} • ${_calculateOverdueDays(issue.dueDate)} days overdue',
                            ),
                          ),
                        ),
                      )
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () {
              ...
              final totalCopies =
                  int.tryParse(totalCopiesController.text.trim()) ?? 0;
              if (title.isEmpty ||
                  author.isEmpty ||
                  isbn.isEmpty ||
                  totalCopies <= 0) {
                focusAndRevealController(
                  ...
                  title.isEmpty
                      ...
                      : isbn.isEmpty
                      ? isbnController
                      : totalCopiesController,
                );
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(content: Text('Enter valid book details.')),
                );
                return;
              }
              setState(() {
                if (book != null) {
                  ...
                  book.availableCopies = book.availableCopies > totalCopies
                      ? totalCopies
  ...
  void _showIssueDialog(
    ...
    final issueDateController = TextEditingController(
      text: issue?.issueDate ?? DateTime.now().toString().split(' ').first,
    );
    ...
    String selectedBook =
        issue?.bookTitle ??
        book?.title ??
        (_books.isNotEmpty ? _books.first.title : '');
    final selectableBooks = _books
        .where(
          (item) => item.availableCopies > 0 || item.title == issue?.bookTitle,
        )
        .toList();
          ...
          ElevatedButton(
            onPressed: () {
              ...
              if (selectedBook.isEmpty ||
                ...
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(content: Text('Enter valid issue details.')),
                );
                return;
              }
              final selected = _books.firstWhere(
                (item) => item.title == selectedBook,
              );
              if (selected.availableCopies <= 0 &&
                  selectedBook != issue?.bookTitle) {
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(
                    content: Text('Selected book is not available.'),
                  ),
                );
                return;
              }
              setState(() {
                if (issue == null) {
                  ...
                  if (issue.bookTitle != selectedBook) {
                    final previous = _books.where(
                      (item) => item.title == issue.bookTitle,
                    );
                    if (previous.isNotEmpty)
                      previous.first.availableCopies += 1;
                    selected.availableCopies -= 1;
                  }
                  issue
  ...
  void _deleteIssue(BookIssue issue) {
    ...
    setState(() {
      if (!issue.returned && matchingBooks.isNotEmpty)
        matchingBooks.first.availableCopies += 1;
  ...
  void _showFineDialog(BuildContext context, FineRecord fine) {
    final amountController = TextEditingController(
      text: fine.amount.toStringAsFixed(0),
    );
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
          ...
          ElevatedButton(
            onPressed: () {
              ...
              if (amount == null || amount < 0) {
                focusAndRevealController(context, amountController);
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(content: Text('Enter a valid fine amount.')),
                );
                return;
              }
...
class LibraryBook {
  ...
  factory LibraryBook.fromJson(Map<String, dynamic> j) => LibraryBook(
    ...
    totalCopies: j['totalCopies'] as int,
    availableCopies: j['availableCopies'] as int,
  );
}
